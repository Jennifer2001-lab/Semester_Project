function [objective, sol, dual_solution_sub] = solve_subproblem_sequence(loads, uncontrollable, grid, resources, costs, el_price, master_sol, T, S, pv_num, subprob_number, sequence_number)
    ...
    for t=1:T  
        ...
        for scenario=1:S
            ...
            Sbars((t-1)*N+1:t*N, scenario) = Slf;
            Lbar((t-1)*2+1:2*t, scenario) = [sum(real(Slf));sum(imag(Slf))];

            pt = real(Slf); 
            qt = imag(Slf); 
            sbar(2*(t-1)+1:2*(t-1) + 2,scenario) = [pt(1);qt(1)];
            psbar(t,scenario) = pt(1); qsbar(t) = qt(1); 

            Vbar((t-1)*N+1:t*N,scenario) = E;
            Itemp = zeros(NL,1);
            ...
            Ibar((t-1)*NL+1:t*NL,scenario) = abs(Itemp);
        end
    end
    ...
    while converged == 0 && niter < nmax
        ...
        for t=1:T
            for scenario=1:S
                Es = Vbar((t-1)*N+1:t*N,scenario); 
                Ss= Sbars((t-1)*N+1:t*N,scenario); 

                % Voltage Sensitivity
                [K_p,K_com_p,K_q,K_comp_q]=Coeffs_Voltage_Alpha(YY,Ss,Es,Res_nodes_no_slack,slack,nph,alphap,alphaq,E_0);
                % Current Sensitivity
                [Icoeff, Icoeff_complex, Icoeffq,Icoeffq_complex, Currents]=Coeffs_Currents(YYL,YYT,Es,K_com_p,K_comp_q,nph);
                ...
                [C_rp , C_rq, C_xp, C_xq] =Coeffs_Losses(YY, Es, K_com_p, K_comp_q, Res_nodes_no_slack);
...
%         el_bin = sdpvar(7, 1);
        ...
        cons = [cons, (dEh == master_sol.dEh(subprob_number)) : 'dEH'];
...
%             end
            ...
            % Storage constraints accounting for sequence
            cons = [cons, Eh(T+1,scenario) <= dEh + eps];
            cons = [cons, Eh(T+1,scenario) >= dEh - eps];
            cons = [cons, Eh(1,scenario) == 0];
            if master_sol.dEh >= 0
                cons = [cons, Eh(:,scenario) >= 0 - eps, Eh(:,scenario) <= dEh + eps];
            else
                cons = [cons, Eh(:,scenario) <= 0 + eps, Eh(:,scenario) >= dEh - eps];
            end
...
%             end
            ...
            cons = [cons, Eb(2:end,scenario) == Eb(1:end-1,scenario) - c(batteryindices(1:T),scenario) ];
            cons = [cons, Eb(:,scenario) >= Ebmax*0.2, Eb(:,scenario) <= Ebmax]; 

            cons = [cons, c(qindices,:) == 0];
            % Hydrogen Storage
            cons = [cons, Eh(2:end,scenario) == Eh(1:end-1,scenario) -  c(fcindices,scenario)/0.75 - c(elindices,scenario) * 0.75];
            cons = [cons, Eh(:,scenario) >= (1/15)*Ehmax - eps, Eh(:,scenario) <= Ehmax + eps]; 
        ...
        toc
        disp('Time to create model')
          %% Write Objective
        obj=0;
        for scenario=1:S
            obj = obj + 10*6* sum(max(el_price(1:T)) * (Dslackpos(:,scenario) + Dslackneg(:,scenario)));

            obj = obj + 6 * sum(el_price(1:T) * splus(:,scenario) - 1/3* el_price(1:T) *  sminus(:,scenario)); %Sb/1e6
        ...
        dual_solution_sub.dEh = dual(cons('dEH'));
        ...
        for t=1:T
            ...
            for scenario=1:S
                ...
                startposdc = (t-1) * NC + 1;
                Sinj(1) = sol.ps(t,scenario) + 1i * sol.qs(t,scenario);
                Sinj(loads.indices(1)) = -10e3*loads.data(t,scenario)*1/Sb;
                Sinj(loads.indices(2)) = -5e3*loads.data(t,scenario)*1/Sb;
                Sinj(uncontrollable.indices(1)) = 10e3*uncontrollable.data{pv_num}(t,scenario)*1/Sb;
                Sinj(uncontrollable.indices(2)) = 10e3*uncontrollable.data{pv_num}(t,scenario)*1/Sb;

                Sinj(idxbat) = sol.Pb(t,scenario) + 1i * sol.qc(startposqc,scenario);
                Sinj(idxh) = sol.Pfc(t,scenario)-sol.Pel(t,scenario) + 1i * sol.qc(startposqc+1,scenario);
                startnode = (t-1) * N + 1;
                ...
                Ibar((t-1)*NL+1:t*NL,scenario) = abs(Itemp);
                ErrorsI(t,scenario) = norm(abs(Itemp) - abs(sol.I((t-1)*NL+1:t*NL,scenario)));
            end
        end
