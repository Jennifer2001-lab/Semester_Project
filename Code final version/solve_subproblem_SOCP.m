...
%         cons = [cons, Eb(midnight,scenario) >= 0.45 * Ebmax];
        cons = [cons, Eb(T+1,scenario) <= 0.55 * Ebmax];
        cons = [cons, Eb(T+1,scenario) >= 0.45 * Ebmax];
        ...
        % Unit commitment per day
%         for d = 0:6
%             cons = [cons, Pfc((1+d*24:d*24+24),scenario) <= fc_bin(1+d)*M];
%             cons = [cons, Pfc((1+d*24:d*24+24),scenario) >= PFCmax*0.1 - (1 - fc_bin(1+d))*M];
%             cons = [cons, Pel((1+d*24:d*24+24),scenario) <= el_bin(1+d)*M];
%             cons = [cons, Pel((1+d*24:d*24+24),scenario) >= PELmax*0.1 - (1 - el_bin(1+d))*M]; 
%         end
        ...
        cons = [cons, Pfc(2:T, scenario) - Pfc(1:T-1, scenario) <= 0.2*PFCmax];
        cons = [cons, Pfc(1:T-1, scenario) -Pfc(2:T, scenario) <= 0.2*PFCmax];
        
        cons = [cons, Pel(2:T, scenario) - Pel(1:T-1, scenario) <= 0.2*PELmax];
        cons = [cons, Pel(1:T-1, scenario) - Pel(2:T, scenario) <= 0.2*PELmax];  
    ...
    obj = obj + 1e4 * sum(sum(flines)) * 1/S;
