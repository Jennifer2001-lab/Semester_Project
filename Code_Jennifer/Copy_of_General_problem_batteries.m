...
while converged == 0 && niter <= nmax
    ...
    sw_fc = sum(abs(diff(fc_bin)));
    sw_el = sum(abs(diff(el_bin)));
     ...
     for scenario=1:S
        ...
        cons= [cons, c(fcindices,scenario) >= resources.PFCmin*fc_bin];
        cons= [cons, c(elindices,scenario) <= -resources.PELmin*el_bin];
        cons = [cons, c(fcindicesc,scenario) >= resources.QFCmin*fc_bin];
        
        cons = [cons, c(fcindices,scenario) <= resources.PFCmax * fc_bin];
        cons = [cons, c(elindices,scenario) >= -resources.PELmax * el_bin];
        cons = [cons, c(fcindicesc,scenario) <= resources.QFCmax * fc_bin];
   ...
   obj=0;
    ...
    obj = obj + (Ebmax + Pbmax + Qbmax + Sbmax)*400;
    obj = obj + (sw_fc + sw_el);
